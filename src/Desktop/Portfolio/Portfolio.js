...
import './style.css'
const Portfolio = () => {
 const {setIsPortfolioActive , isPortfolioActive ,taskparicons , setTaskparicons , data} = useGlopalContext()
 ...
 const reverseProjects = [...data].reverse()
 
 return (
  <div className={`${isPortfolioActive ? 'window-container active':'window-container'}`}>
   <div className="header"> 
   <div className="headerbuttons">
    <button className=' heder-icon'  onClick={()=>setIsPortfolioActive(false)}><FaRegWindowRestore/></button>
    <button className=' heder-icon' onClick={()=>closePortfoliolWindow()}><FaTimes/></button>
   </div>
   </div>
   {/* /////////// Header End ////////////////// */}
       ...
       reverseProjects.map((project)=>{
        ...
        return(
         <div className='project-container'key={id}>
          ...
          <footer id='project-id'><p>{title}</p></footer>
         </div>
         </a>
         </div>
        )
