...
export function Dock() {
  ...
  return (
    <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-50 select-none">
      <div className="glass-dock px-3 py-2 rounded-2xl flex items-center space-x-2 border border-slate-700/60 shadow-2xl bg-slate-900/80 backdrop-blur-2xl">
        {dockItems.map((item) => {
          ...
          return (
            <div key={item.id} className="relative group flex flex-col items-center">
              {/* Tooltip */}
              <div className="absolute -top-10 scale-90 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-150 pointer-events-none bg-slate-900 border border-slate-700 text-slate-200 text-xs font-medium px-2.5 py-1 rounded-lg shadow-xl whitespace-nowrap">
                {item.label}
              </div>
              ...
              <button
                ...
                className={`w-12 h-12 rounded-xl flex items-center justify-center border transition-all duration-200 transform group-hover:-translate-y-2 group-hover:scale-110 shadow-lg ${
                  item.colorClass
              ...
              {/* Indicator Dot */}
              <div className="h-1.5 w-1.5 rounded-full mt-1 transition-all duration-200">
                {isActive ? (
                  <div className="w-2.5 h-1.5 rounded-full bg-cyan-400 shadow-sm shadow-cyan-400" />
                ) : isOpen ? (
        ...
        {/* Divider */}
        <div className="h-9 w-px bg-slate-700/80 mx-1" />
